...
- (void)setData:(NSArray *)titles NormalColor:(UIColor *)n_color SelectColor:(UIColor *)s_color Font:(UIFont *)font{
    ...
    for (int i=0; i<titles.count; i++) {
        ...
        if (i == 0) {
            ...
            self.lineView.backgroundColor = [UIColor colorWithRed:253/255.0 green:209/255.0 blue:62/255.0 alpha:1];
            self.lineView.layer.masksToBounds = YES;
            [self addSubview:self.lineView];
        }
        UILabel * label = [[UILabel alloc]init];
        [label setFrame:CGRectMake(i*_width+_width-15, 5, 12, 12)];
        if (self.numberArr.count != 0) {
            [label setText:self.numberArr[i]];
        }else{
            [label setText:@"0"];
        }
        ...
        [label setBackgroundColor:[UIColor colorWithRed:253/255.0 green:209/255.0 blue:62/255.0 alpha:1]];
